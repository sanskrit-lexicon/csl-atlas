...
- **Provenance gap** — F9 reproduces exactly (12.336 / 1.51, frozen `key1` exports); F1/F5 drift (587 → 598, 3,593 → 3,583) — most plausibly upstream drift, not proven exclusive, because no sidecar pins the `csl-orig` revision; the parsed cache's own generating revision is unrecorded too (`parse_cslorig.py` now writes `_parse_provenance.json` for future builds). **Closed by H5248 (22-09-2026):** the F1/F5 figures were re-frozen at `csl-orig@f4c08c57`, every F1/F5 sidecar records it, and a second fresh run reproduces them to the digit.

## Ranked improvement backlog

1. **Pin `csl-orig` in every `.source.json` sidecar** and re-freeze the F1/F5/F10 figures against one named revision — the cleanest fix for §4's gap, and it moves every `f*_report.json` at once. — *status: done for F1/F5 (H5248, 22-09-2026, `csl-orig@f4c08c57`; a second fresh run reproduces to the digit). F10's writer records the revision, but F10 was not re-run. Other `f*` sidecars (F2–F4, F8) still pin only the csl-atlas commit — open, cheap.*
...
- No claim was refuted. The audit's output is a constraint on how the signals may be *combined*. The only published figures revised are the F1/F5 digits re-frozen at a recorded revision (H5248, A10 §7). No verdict changed.
