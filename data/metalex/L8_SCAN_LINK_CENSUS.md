...
| Atlas-resolvable Cologne scan URL | 1,186,408 | 1,496,157 | **79.30** |

**L8 complete?** **No.** Do not claim L8 complete: print coordinates are nearly universal, but 7 dicts with <pc> data still have no verified Cologne scan-dir map (309,748 entries), so a working scan URL resolves for 79.30% of entries under the atlas's verified dict→scan-dir map.
...
| `pc_present_no_cologne_scan_dir` | 309,748 | `<pc>` present but dict not in atlas `COLOGNE_SCAN_DIR` (no verified Cologne scan URL builder) |
...
- **In atlas `COLOGNE_SCAN_DIR`:** `abch, acph, acsj, ae, ap, ap90, armh, ben, bhs, bop, bor, bur, cae, ccs, fri, gra, gst, ieg, inm, krm, lan, lrv, mci, md, mw, mw72, mwe, pe, pgn, pwg, sch, shs, snp, stc, vcp, wil, yat`
- **Atlas-resolvable 100%:** `abch, acph, acsj, ap, ap90, armh, ben, bhs, bop, bor, bur, cae, ccs, fri, gra, gst, ieg, inm, krm, lan, lrv, mci, md, mw, mw72, mwe, pe, pgn, pwg, sch, shs, snp, stc, vcp, wil, yat` (ap90 reached 100% in two steps: page-column-letter shape `NNNN-a/b/c` stripped to the page — H2368-A10 fix; then the page-column-digit shape `NNNN-N` seen at new-letter section breaks, e.g. `0220-1`, `0351-2` — H2368-A11 follow-up, same reasoning: the trailing chunk is a column marker, not a volume)
- **In scan-dir map but not 100% resolvable:** `ae`
- **Not in scan-dir map:** 7 dicts (dominant gap — coordinates exist; atlas has no verified servepdf map)
...
| ap | 90,843 | 90,843 | 100.00 | 90,843 | 100.00 | yes | `N-N`×90094, `N-aN`×693, `N-bN`×50 |
| mw72 | 55,390 | 55,390 | 100.00 | 55,390 | 100.00 | yes | `N-a`×18569, `N-c`×18519, `N-b`×18302 |
| lrv | 53,441 | 53,441 | 100.00 | 53,441 | 100.00 | yes | `N-N`×53221, `N-N.N`×220 |
