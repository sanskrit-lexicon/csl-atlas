...
const REPO_BLOB_BASE = "https://github.com/sanskrit-lexicon/csl-atlas/blob/main";

function docsHref(file) {
  // Full blob URL (repo link convention): the same link resolves from docs/,
  // test/fixtures/ and the GitHub rendering, so link-hygiene passes have
  // nothing to rewrite in this generated output.
...
export function buildMarkdown(payload) {
  const lines = [
    ...
    "",
    "_Generated by `node scripts/build-r2-checkpoint-packet.mjs` — do not hand-edit; regenerate with `npm run regen-review-artifacts`._",
    "",
    "Date: 2026-06-06",
    "",
  ...
  for (const [packetId, context] of Object.entries(payload.packetContext)) {
    ...
    lines.push(
      ...
      `Packet: \`${packetId}\``,
      "",
      `Vocabulary doc: ${context.vocabularyDoc ? `[\`${context.vocabularyDoc}\`](${docsHref(context.vocabularyDoc)})` : "n/a"}`,
      "",
