...
| H5 | Type 2 | Ghost entries and rare anomalies identify both lineage and QA targets. | Proof-first review queue and taxonomy are active: 130 F0/F2/known-correction rows preserve review fields, with all rows still `needs-review` and no error claims before source review. | [`H5_GHOST_ANOMALY_SCOPE.md`](H5_GHOST_ANOMALY_SCOPE.md), `src/data/review/h5-anomaly-review.json`, [`REVIEW_QUEUE_PROOFS.md`](REVIEW_QUEUE_PROOFS.md) | `csl-atlas` | Classify known corrections and null controls first, then raw-headword-exclusive and shared-doublet rows. | Maker QA worklist; forensic methods note. |
