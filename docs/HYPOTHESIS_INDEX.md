...
| H5 | Type 2 | Ghost entries and rare anomalies identify both lineage and QA targets. | Proof-first review queue and taxonomy are active: all 130 rows are `reviewed-ok`, the generated maker-QA packet has 10 source-checked rows, and only `divaraTa -> diviraTa` is marked as a source-declared correction candidate. | [`H5_GHOST_ANOMALY_SCOPE.md`](H5_GHOST_ANOMALY_SCOPE.md), [`H5_MAKER_QA_CANDIDATES.md`](H5_MAKER_QA_CANDIDATES.md), `data/lexico/h5_maker_qa_candidates.json`, `src/data/review/h5-anomaly-review.json`, [`REVIEW_QUEUE_PROOFS.md`](REVIEW_QUEUE_PROOFS.md) | `csl-atlas` | Draft a maker-facing correction proposal for `divaraTa -> diviraTa`; keep the other nine source-supported rows out of correction flow. | Maker QA worklist; forensic methods note. |
