...
| H5 anomaly review | `src/data/review/h5-anomaly-review.json`; `data/lexico/h5_maker_qa_candidates.json` | F0/F2 forensic candidates can be separated into lineage signals, correction targets, parser artifacts, null controls, and legitimate morphology. | It does not prove any queued headword is wrong; the first pass accepted no ghost candidates, and the source-check dispositions are review metadata rather than dictionary edits. | Draft a maker-facing proposal for the single source-declared `divaraTa -> diviraTa` correction candidate. |
