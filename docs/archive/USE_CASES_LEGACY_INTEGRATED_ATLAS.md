...
inventory. The active dictionary-only use cases are in `docs/USE_CASES.md`.

Date: 2026-05-29

Status: legacy broad use-case inventory. `docs/BOUNDARY_RULES.md` now controls
scope: atlas work must start from dictionary evidence. Standards/export use
cases belong in `csl-standards`; DCS/corpus work belongs elsewhere.

## Purpose

The atlas now centers the dictionary-evidence domain:

```text
dictionary evidence -> source link -> review status
```
...
| Data reuse mode | How can I reuse the generated dictionary data elsewhere? | compact JSON, schemas, and provenance |
...
### UC-RV-06 Standards Review Lives Elsewhere `P1`

- Actor: TEI or semantic web reviewer.
- Goal: review TEI/OntoLex/RDF standards outputs.
- Atlas response: external pointer to `csl-standards`.
- Evidence: standards workbench validation reports.
...
### UC-DEV-01 Standards Pipeline Lives Elsewhere `P0`

- Actor: developer.
- Goal: rebuild or validate TEI/OntoLex/FrAC standards outputs.
- Atlas response: external pointer to `csl-standards`; no atlas-local standards commands.
- Evidence: `C:\Users\user\Documents\GitHub\csl-standards`.
...
TEI, OntoLex, FrAC, SHACL, RDF, and standards/export use cases now belong in `csl-standards`, not in `csl-atlas`.
...
- Actor: author.
- Goal: support claims about dictionary evidence and dictionary comparison.
- Atlas response: source-linked dictionary dashboards, review labels, and caveats.
- Evidence: dictionary source records and generated atlas summaries.
