# H5 Maker Proposal — divaratha → diviratha (MW, PWG), with the evidence packet
...
_Encoding note (for non-Cologne readers): citations below use the Cologne/csl-orig
internal encoding, SLP1 — capital `T` = th, `w` = ṭ, `A` = ā, `I` = ī (per the atlas's
own mapping, [src/lib/sanskrit-util.js](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/src/lib/sanskrit-util.js):
`w: 'ṭ'`, `T: 'th'`). So `divaraTa` = divaratha (दिवरथ) and `diviraTa` = diviratha
(दिविरथ) — the name Sørensen's index writes "Diviratha". An earlier draft of this
document misread the keys as Kyoto-Harvard ("ṭa"); corrected 24-09-2026 on verifier
review._
...
Two Cologne digital dictionary entries carry the headword `divaraTa` (divaratha):
...
Both entries declare — in their own words — that divaratha is the **wrong reading** for
diviratha (`w.r.` = wrong reading; `falsche Form` = wrong form). Neither carries any
independent lexical content: no sense, no analysis, only the pointer to diviratha. Every
independent witness in the Cologne estate (PW, Vācaspatyam, Sørensen's Index of Names in
the Mahābhārata) and the DCS corpus records **only** diviratha; divaratha occurs nowhere.
...
layer can redirect or alias a `divaratha` lookup to the `diviratha` entry content, we
...
| Dictionary | divaratha | diviratha | Entry record | Body (transliterated) | Printed locus (`<pc>`) |
|---|---|---|---|---|---|
...
| INM (Sørensen, An Index to the Names in the Mahābhārata) | — | ✓ | [inm.txt L3636](https://github.com/sanskrit-lexicon/csl-orig/blob/master/v02/inm/inm.txt#L43478), [L3637](https://github.com/sanskrit-lexicon/csl-orig/blob/master/v02/inm/inm.txt#L43483) | two entries for `diviraTa` (two homs); the SLP1 key `raTa` = ratha, and Sørensen writes the name "Diviratha" in the body — "Diviratha, the son of Bhumanyu. § 152" | `<pc>256-1` (p. 256) |
...
diviratha; divaratha exists only in MW and PWG, and in both of those the wrong form is
flagged by the dictionary itself.** Note also that MW's own diviratha entry already
...
|---|---|---|
| MW, divaratha (L92243) | p. 478, col. 3 | [MWScan p. 478, side b](https://www.sanskrit-lexicon.uni-koeln.de/scans/MWScan/2020/web/index.php?page=478&side=b) |
| MW, diviratha (L92364) | p. 479, col. 1 | [MWScan p. 479, side a](https://www.sanskrit-lexicon.uni-koeln.de/scans/MWScan/2020/web/index.php?page=479&side=a) |
| PW, diviratha (L50335) | vol. 3, p. 89, col. b | [PWScan p. 89, side b](https://www.sanskrit-lexicon.uni-koeln.de/scans/PWScan/2020/web/index.php?page=89&side=b) |
| PWG, divaratha (L32945) | vol. 3, p. 620 | [PWGScan p. 620](https://www.sanskrit-lexicon.uni-koeln.de/scans/PWGScan/2020/web/index.php?page=620&side=a) |
| PWG, diviratha (L33004) | vol. 3, p. 623 | [PWGScan p. 623](https://www.sanskrit-lexicon.uni-koeln.de/scans/PWGScan/2020/web/index.php?page=623&side=a) |
| VCP, diviratha (L25025) | `<pc>3588,a` | [VCPScan p. 3588](https://www.sanskrit-lexicon.uni-koeln.de/scans/VCPScan/2020/web/index.php?page=3588&side=a) |
| INM, diviratha (L3636/L3637) | p. 256 | [INMScan p. 256](https://www.sanskrit-lexicon.uni-koeln.de/scans/INMScan/2020/web/index.php?page=256&side=a) |
...
- Bhāgavata Purāṇa 9,23,6 — diviratha as son of Khanapāna, per PWG L33004.
...
## 6. Reasoning: why divaratha is a source-declared wrong form

1. `observed` — MW L92243 says `w.r. for divi-`; PWG L32945 says `falsche Form für
   diviraTa`. The dictionaries themselves declare it.
2. `derived` — at the pinned csl-orig revision, divaratha is a headword in exactly two
   of the six witnesses checked (MW, PWG); PW, VCP, INM, and DCS carry only
   diviratha.
3. `observed` — MW L92364 registers the variant reading `v.l. diva-` inside the
   diviratha entry itself.
4. `derived` — DCS-2021: 0 occurrences of divaratha; diviratha attested (rare band).
5. `observed` — PWG L33004 gives the only morphological analysis in play:
   diviratha = divi (locative of div "sky") + ratha (m. "car"); no analysis of
   "divaratha" exists anywhere.
6. `derived` — the atlas cross-reference layer already records the PWG-internal
   `Vgl.` edge between the two entries:
   [data/lexico/xref_edges.csv](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/lexico/xref_edges.csv)
   row `pwg,33004,diviraTa,vgl,divaraTa`.
...
`diviraTa` — with the detector's nearest neighbour `devaraTa` (`deva-ratha` "god's car",
