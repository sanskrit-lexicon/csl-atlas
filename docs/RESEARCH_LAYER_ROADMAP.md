...
| **H6** | **Structural register (citation × grammar-marking) predicts tradition family.** | cluster the macro profile (§1.2); compare to the genealogy tree | researchers | prototype supported, charted, review-classed against L0 known edges, and documented as a rerunnable method note |
...
- **[x]** H6 structural review artifact and packet — compares H6 chart distance with L0 known-edge support, generated by `npm run build-h6-structural-review` and documented in `H6_STRUCTURAL_REGISTER_REVIEW.md`.
