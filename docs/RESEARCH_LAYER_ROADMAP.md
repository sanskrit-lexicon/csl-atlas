...
*v1.2: historical round-2 decisions (§7) — external DCS/VisualDCS ownership,
main-dashboard hosting, the initial sense-granularity test, and
Amarakośa-native field scheme; DCS ownership superseded by v1.3 boundary note.*
*v1.3 boundary update 2026-06-04: DCS/corpus frequency joins are external
VisualDCS work, not atlas generation work.*
*v1.4 hypothesis update 2026-06-04: H1/H3 negative results and H2/H6 positive
results folded into a cross-repo hypothesis index.*
**Companion to**: [`BOUNDARY_RULES.md`](BOUNDARY_RULES.md), [`HYPOTHESIS_INDEX.md`](HYPOTHESIS_INDEX.md), [`LEXICOGRAPHY_ROADMAP.md`](LEXICOGRAPHY_ROADMAP.md) (genealogy/phylogeny), [`MICROSTRUCTURE-MACROSTRUCTURE.md`](MICROSTRUCTURE-MACROSTRUCTURE.md) (structure typology), [`METALEXICOGRAPHY_ROADMAP.md`](METALEXICOGRAPHY_ROADMAP.md).
...
## 2. Hypothesis status after R2 and M1-M4

Canonical cross-repo sorting now lives in
[`HYPOTHESIS_INDEX.md`](HYPOTHESIS_INDEX.md). This section keeps the original
H1-H7 research stream readable, but the status labels now reflect the evidence
already generated by R2 sense-splitting and the microstructure work.

| # | Claim | Signal / method | Audience | Status |
|---|---|---|---|---|
| **H1R** | **Sense granularity is a family/marking-style trait, not pure temporal inflation.** | sense-units per entry vs publication year, family, and marking style | researchers, makers | **finding**: original H1 is not supported; see [`R2_FINDINGS.md`](R2_FINDINGS.md) |
| **H2** | **Citation density predicts a sense's survival** into later dictionaries. | cited vs uncited ancestor-sense survival on inheritance edges | researchers | **supported finding**: cited 70% vs uncited 54%; broaden next |
| **H3R** | **Descendants copy or condense senses unless edge-specific evidence proves expansion.** | net sense delta and gloss overlap along known inheritance edges | researchers, historians | **finding**: original net-addition H3 is not supported; see [`R2_FINDINGS.md`](R2_FINDINGS.md) |
| **H4** | **Each dict has a measurable semantic-field bias** (ritual / grammar / flora / law / medicine). | classify glosses into **Amarakośa-native topical fields** → per-dict field distribution | researchers, students | **strong testable hypothesis**: scheme decided; implementation pending |
| **H5** | **"Ghost entries"** — shared OCR/typo anomalies — are both a lineage fingerprint **and** an editor QA flag. | rarity-weighted shared-anomaly detection (extends L3 forensic) | makers, historians | **strong testable hypothesis**: anomaly scope still open |
| **H6** | **Structural register (citation × grammar-marking) predicts tradition family.** | cluster the macro profile (§1.2); compare to the genealogy tree | researchers | **prototype-backed finding** |
| **H7** | **First-N sampling materially biases structure metrics** (early-alphabet entries are shorter/sparser). | compare first-N vs random vs stratified samples on the same dicts | methodology | **resolved finding**: full corpus / stratified measurement is the default |
...
directly. H6 is a ready figure; R2 has already turned H1/H2/H3 into the
H1R/H2/H3R findings now tracked in [`HYPOTHESIS_INDEX.md`](HYPOTHESIS_INDEX.md).
...
| **R2** | **Sense splitter** per dict format → sense-level corpus — *first slice shipped 2026-05-31* ([R2_FINDINGS.md](R2_FINDINGS.md)) | dict format study (have micro typology) | H1R/H2/H3R, sense-alignment, divergence map |
...
The **sense splitter (R2)** is the critical dependency for sense-alignment,
divergence views, and the maker worklist. Its first slice already produced the
H1R/H2/H3R findings; broader parser coverage is the next investment.
...
known inheritance edges PWG→MW72→MW and AP90→AP used by the sense findings) →
reverse-direction (small, clean) → indigenous-quotation (hardest) → indexes
excluded. **Output:** a sense-level corpus `data/lexico/senses_<dict>.jsonl`
(one record per sense: dict, lemma, sense-index, gloss-span,
Sanskrit-fingerprint), feeding the sense-alignment view and the divergence map
(the maker worklist).
...
- **Sense parsing is hard.** The H1R/H2/H3R findings, alignment view, and
  divergence map all need broader per-dict sense-splitter coverage (R2) before
  they become robust public tools.
...
- **First hypothesis after R2** → completed as **H1R**: pure sense inflation
  over editorial time is not supported; sense granularity is better treated as
  a family/marking-style trait.
...
**Still open:** anomaly/ghost-entry detector scope (H5); whether the
H1R/H2/H3R sense findings are their own short paper or a Paper-L section;
co-author assignment for the sense-evolution work.
