...
- **[x]** Sense-alignment view — senses of one lemma *aligned* across dicts (like a sequence alignment), not just side-by-side; highlights where dicts agree/diverge/add senses. Shipped as the "Sanskrit sense-alignment explorer" (`/tools/r2-explorer`, `src/tools/r2-explorer.md`), generated from `data/lexico/r2_align_*.json` (5 R2 anchor lemmas: dharma/gam/iti/rāma/bodhisattva), with Sanskrit-anchored (no-translation) cross-dictionary alignment (verified 29-07-2026, H1877 roadmap-drift sweep).
- **[ ]** Sense-provenance timeline — each sense's earliest attesting dict + its citation era.
- **[x]** Entry-anatomy radar — per-dict overlay of the microstructure feature matrix (10 features actually implemented, not the aspirational 24) for one entry at a time, generated by `npm run build-micro-entry -- <lemma>` over the R2 anchor lemmas (gam/dharma/rāma/iti); rendered at `/tools/entry-anatomy` (H1510).
