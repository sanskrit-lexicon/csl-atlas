...
- Next use: review the generated R2 checkpoint packet before any parser
  promotion; record drift explanations only after human decisions exist.
...
next accepted action is to review the ten-row checkpoint packet in
...
experiment against the archived fixtures. Before testing parser changes, review
the checkpoint packet generated by `npm run build-r2-checkpoint-packet`.
Parser changes can be considered only after those checkpoint rows have human
decisions. The later parser experiment order remains:
