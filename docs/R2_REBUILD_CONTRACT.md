...
- Evidence: archived findings in `R2_FINDINGS.md`, static pages
  ...
  `docs/R2_CHECKPOINT_REVIEW.md` and `docs/R2_CHECKPOINT_DECISIONS.md`.
...
- Next use: record checkpoint decisions in the shared R2 review report before
  any parser promotion; record drift explanations only after human decisions
  exist.
...
labels, not scholar-reviewed sense decisions or `reviewedValue` fields. The
next accepted action is to record decisions in the shared checkpoint review
report, guided by [`R2_CHECKPOINT_DECISIONS.md`](R2_CHECKPOINT_DECISIONS.md)
and [`R2_CHECKPOINT_REVIEW.md`](R2_CHECKPOINT_REVIEW.md), keeping parser
promotion deferred until human decisions exist.
...
Use the source-backed anchor prototype to prepare for a non-final rebuild
experiment against the archived fixtures. Before testing parser changes, record
human decisions in the checkpoint report generated by
`npm run build-r2-checkpoint-review`. Parser changes can be considered only
...
The later experiment should emit a drift explanation artifact before any public
R2 claim changes. Broaden beyond `gam`, `dharma`, `rama`, `iti`, and
