_Created: 08-07-2026 · Last updated: 16-09-2026_
...
- **Source files:** anchor source (H4720 consumer edge)
  [`kosha/data/db/kosha.db`](https://github.com/gasyoun/kosha) `heritage_anchor`
  table (${totals.crosswalkRows.toLocaleString()} rows — kosha's H345 ingest of
  the same crosswalk, one row per raw `mw_key1`, consumed read-only); upstream
  ...
  owned by SanskritLexicography; also the documented fallback when a sibling
  kosha checkout is absent, e.g. CI) joined onto MW's `<k1>` headwords from
  ...
  with kosha-commit + crosswalk-commit provenance in its `.source.json` sidecar.
- **Generated by:** `npm run build-heritage-witness`
- **Validation:** `npm run validate-heritage-witness` — totals arithmetic,
  per-initial sums, witnessed-row tier consistency, a full rebuild
  cross-check against the sibling crosswalk + live MW dict when the checkout
  is present, and (H4720) a deterministic ~400-row sample of witnessed rows
  re-verified value-for-value against the kosha `heritage_anchor` table when
  the sibling kosha checkout is present.
...
- **Next action:** none open on the anchor source — the kosha `heritage_anchor`
  serving layer is the live feed since H4720 (edge
  `csl-atlas->kosha · heritage_anchor witness anchors`, live in Uprava
  `interlinks_edges.tsv`). A follow-on could add a "corpus-attested AND
  Heritage-covered" combined axis here.
- **External dependencies:**
  [`kosha`](https://github.com/gasyoun/kosha) (heritage_anchor serving layer),
  [`SanskritLexicography`](https://github.com/gasyoun/SanskritLexicography)
  (upstream crosswalk), csl-orig v02 (MW headwords).
