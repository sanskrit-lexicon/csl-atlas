...
(dictionaries carry partly disjoint tradition communities)? The verdict is
modular; the partition found is two key-sharing pairs (`ap`+`ap90`, `pwkvn`+`sch`)
plus singletons, and the margin over the null survives variant folding — see the
[H5295 claim ledger](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/A50_CLAIM_FALSIFICATION_LEDGER.md)
for what that does and does not identify.
...
const verdictLabel = {
  ...
  "modular": "Modular (beyond one canon)",
...
## Tradition profiles — what the map shows alongside the modular split

The topology verdict above says the apparatus is **modular** — it clusters beyond
what a one-canon draw forces. This panel gives each dictionary a tradition *profile*
(it does not name the optimiser's modules, which are two key-sharing pairs and seven
singletons), joining a
...
> as almost purely Buddhist, the Apte pair (`ap`/`ap90`) and `lrv` as
> classical-kāvya, `mw`/`md` as Vedic (over a small tagged sample — see the
> limitations below). These are profiles per dictionary, not the optimiser's
> modules: the modular split itself is two key-sharing pairs plus singletons
> (H5295 ledger, C4).
...
- Review status: the topology test is machine-reviewed; the tradition-profile panel is built from the curated [`tradition_tags.tsv`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/citations/tradition_tags.tsv) map (`inferred`, agenda backlog #9), flagged by review state on the page — unreviewed tags are never shown as fact. Vote via `npm run build-tradition-review-sheet` → `review/csl-atlas-tradition-tags_119texts_review.html`.
