...
- Evidence: `src/data/block-by-type-matrix.json` generated from MW article-type and block classifiers.
- Limitations: differences are structural block percentages, not semantic explanations or source-dependent claims.
- Validation: generated by `npm run build-mw-depth`; checked by `npm run validate-mw-depth`, `npm test`, and `npm run build`.
- Owner repo: `csl-atlas`.
- Next use: compare the selected types, then inspect examples before treating a difference as structural.
