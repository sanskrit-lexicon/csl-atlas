...
Start with Which dictionary should I use? if you need a
...
lookup for dictionary-first search across MW, AP, PWG,
...
Dictionary genealogy, convention fingerprints, structural register, etymology style (WIL Nirukta vs Western `cf.`), and R2 sense alignment / sense granularity move dictionary-structure research into the atlas path.
...
- **Lexicographic timeline** — inventory lifespan bars, family lanes, lemma size over time, coverage ribbon (Mermaid narrative below the fold)
...
[VisualDCS](https://github.com/gasyoun/VisualDCS) provides DCS corpus dashboards and per-lemma frequency data (M1–M8 CoNLL-U→SQLite pipeline). Its `dcs_lemma_summary.json` feed (83,239 lemmas, DCS-2021 corpus release; consumption terms in [`docs/VISUALDCS_CONSUMPTION_CONTRACT.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/docs/VISUALDCS_CONSUMPTION_CONTRACT.md)) is wired into this atlas: the Dictionary Dossier shows a per-lemma DCS frequency-band chip (1 = hapax … 5 = very common, log₁₀ orders), and the learner reading layer folds the same bands into its index. Follow the VisualDCS link above for the full corpus dashboards.
