...
**Striking absence: `<ls>L.</ls>` = 0**. PWG never collapses kosha sources into a generic hedge — it always names which kosha. The top 5 sigla are all named indigenous Sanskrit lexicons (ŚKDR, MED, H. an., RĀJAN, ŚABDAR); together they account for 42,931 citations (7.5 % of PWG's total apparatus). MW's `<ls>L.</ls>` 40,212-instance hedge ([MW chapter §4](mw#4-hedge-analysis--the-lsl-construct)) is the editorial compression of *exactly this kind of named-kosha citation* into a binary "named-source vs lexicographer-only" distinction.
...
| **next →: [PWK](pwk)** | Same editor (Böhtlingk) without Roth; structurally identical record format; same `<ls>` tag system | PWK drops PWG's kosha apparatus almost entirely (PWG `<ls>ŚKDR.</ls>` 20,109 → PWK 0; PWG `<ls>H. an.</ls>` 6,619 → PWK 0); PWK single-volume forces type-citation spread to 7.7 pts |
...
- **Per-dict matrix JSON**: [`data/pwg_blocks.json`](../data/pwg_blocks.json) (regenerable via `export_dict_blocks.py PWG`)
